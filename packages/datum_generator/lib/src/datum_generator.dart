...
class DatumGenerator extends GeneratorForAnnotation<DatumSerializable> {
  ...
  dynamic generateForAnnotatedElement(
    ...
    final strictNullChecks = annotation.read('strictNullChecks').isNull
        ? false
        : annotation.read('strictNullChecks').boolValue;
    ...
    // Determine the primary constructor's parameter names so generated
    // constructor calls never pass non-constructor fields (#26).
    final ctorParamNames = _getConstructorParamNames(element);

    // copyWith
    _generateCopyWith(buffer, className, allFields);

    // copyWithAll
    _generateCopyWithAll(buffer, className, allFields, ctorParamNames);
  ...
  void _generateDatumMixin(
    ...
    // Generated by default; harmless if a hand-written entity does not apply it.
    ...
    for (final field in allFields) {
      if (_hasRelationAnnotationOnField(field)) {
        ...
        if (fieldName.startsWith('_')) {
          ...
          buffer.writeln('    return (this as $className).$fieldName;');
          buffer.writeln('  }');
          ...
          buffer.writeln('      relations[\'$publicName\']?.setRaw(value);');
          buffer.writeln('    }');
          buffer.writeln('  }');
        }
      }
  ...
  void _generateToDatumMap(StringBuffer buffer, List<dynamic> fields) {
    ...
    buffer.writeln("      map['created_at'] = createdAt.toIso8601String();");
    buffer.writeln("      map['modified_at'] = modifiedAt.toIso8601String();");
    buffer.writeln('    } else {');
    buffer.writeln(
      "      map['created_at'] = createdAt.millisecondsSinceEpoch;",
    );
    buffer.writeln(
      "      map['modified_at'] = modifiedAt.millisecondsSinceEpoch;",
    );
    buffer.writeln('    }');
  ...
  void _generateDiff(
    ...
    for (final field in fields) {
      ...
      final compareExpr = (needsDeepEqual && isCollection)
          ? '!isEqual($fieldName, old.$fieldName)'
    ...
    if (fields.any((f) => _getElementName(f) == 'modifiedAt')) {
      ...
      buffer.writeln(
        "      changes['modified_at'] = modifiedAt.toIso8601String();",
      );
  ...
  /// Best-effort set of the primary constructor's parameter names.
  ///
  /// Used so generated constructor calls (`copyWithAll`, `fromMap`) never pass
  /// arguments for fields that are NOT constructor parameters — e.g. a `userId`
  /// assigned via `: userId = id` — which would produce invalid code (#26).
  /// Returns an empty set when it cannot be determined (analyzer version
  /// differences), in which case callers fall back to passing all fields.
  Set<String> _getConstructorParamNames(dynamic element) {
    List<dynamic>? ctors;
    try {
      ctors = (element.constructors as List?)?.toList();
    } catch (_) {
      try {
        ctors = (element.constructors2 as List?)?.toList();
      } catch (_) {
        return {};
      }
    }
    if (ctors == null || ctors.isEmpty) return {};
    ...
    List<dynamic>? params;
    try {
      params = (ctor.parameters as List?)?.toList();
    } catch (_) {
      try {
        params = (ctor.formalParameters as List?)?.toList();
      } catch (_) {
        return {};
      }
    }
    final names = <String>{};
    for (final p in params ?? const []) {
      try {
        final n = p.name;
  ...
  void _generateCopyWithAll(
    ...
    String className,
    List<dynamic> allFields,
    Set<String> ctorParamNames,
  ) {
    // When constructor params are known, a field can only be a copyWith arg if
    // the constructor actually accepts it. Otherwise fall back to all fields.
    final hasCtorInfo = ctorParamNames.isNotEmpty;
    bool isCtorParam(String name) =>
        !hasCtorInfo || ctorParamNames.contains(name);

    // Only fields that are NOT ignored, NOT relations, and ARE constructor params.
    final copyableFields = allFields
        .where(
          (f) =>
              !_hasRelationAnnotationOnField(f) &&
              !_isIgnored(f, property: 'copyWith') &&
              isCtorParam(_getElementName(f)),
        )
        .toList();
    ...
    for (final field in allFields) {
      ...
      final fieldName = _getElementName(field);
      // Skip fields that are not constructor parameters (they are derived in the
      // constructor body/initializer and cannot be passed).
      if (!isCtorParam(fieldName)) continue;
      ...
      if (fieldName == 'version') {
        ...
        // A constructor param that is ignored from copyWith: pass the current
        // value (the bare name resolves to the instance field here).
...
''');
    ...
    for (final field in allFields) {
      ...
      // Skip fields that are not constructor parameters (e.g. derived via an
      // initializer like `: userId = id`) — they cannot be passed to the
      // constructor (#26).
      if (!isCtorParam(fieldName)) continue;
      ...
      if (fieldName == 'createdAt' ||
        ...
        buffer.writeln(
          "    $fieldName: (map['$mapKey'] ?? map['$fieldName']) != null ? _${_toLowerCamelCase(className)}ParseDate(map['$mapKey'] ?? map['$fieldName']) : null,",
        );
      } else if (type == 'int') {
        buffer.writeln(
          strictNullChecks
        ...
        buffer.writeln(
          strictNullChecks
              ? "    $fieldName: ((map['$mapKey'] ?? map['$fieldName']) as num).toDouble(),"
  ...
  String? _getRelationInfoFromField(dynamic field) {
    try {
      ...
      if (manyToMany != null) {
        ...
        final thisForeignKey = _camelToSnake(
          reader.read('thisForeignKey').stringValue,
        );
        final otherForeignKey = _camelToSnake(
          reader.read('otherForeignKey').stringValue,
        );
        final thisLocalKey = _camelToSnake(
          reader.read('thisLocalKey').stringValue,
        );
        final otherLocalKey = _camelToSnake(
          reader.read('otherLocalKey').stringValue,
        );
        final cascadeDelete = reader.read('cascadeDelete').stringValue;
